...
def draw_menu(ctx: dict, menu: dict) -> None:
    ...
    while key != 27 and key != 113:
        ...
        if key == 456 or key == 115 or key == curses.KEY_DOWN:
            active = (active + 1) % len(options)
        elif key == 450 or key == 119 or key == curses.KEY_UP:
            active = (active - 1) % len(options)
        elif key == 10 or key == 101:
            return options[active][CALLBACK]
